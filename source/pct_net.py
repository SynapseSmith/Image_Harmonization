...
class PCTNet(nn.Module):
    ...
    def forward(self, image, image_fullres=None, mask=None, mask_fullres=None, backbone_features=None):
        ...
        for id, fr_img, fr_mask in zip(idx, fr_imgs, fr_masks):
            ...
            # Attention map과 Full resolution 크기 일치
            if self.use_attn:
                ...
                # fr_mask의 크기를 fr_img에 맞춤
                # print(fr_mask)
                # print(fr_img)
                if fr_mask.size() != fr_img.size():
                    fr_mask = F.interpolate(fr_mask, size=(H, W), mode='nearest')
